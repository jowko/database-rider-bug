...
public final class ConnectionManager
{
    ...
    public static String getConfiguredDataSourceBeanName( ExtensionContext extensionContext )
    {
        ...
        {
            Optional< DBRider > annotation =
                AnnotationUtils.findAnnotation( testMethod.get(), DBRider.class );
            if( !annotation.isPresent() )
            {
                annotation =
                    AnnotationUtils.findAnnotation( extensionContext.getRequiredTestClass(), DBRider.class );
            }
            return annotation.map( DBRider::dataSourceBeanName )
                .orElse( EMPTY_STRING );
        }
        else
        {
    ...
    public static ConnectionHolder getConnectionHolder( String executorId, DataSource dataSource )
    {
        try
        {
            ...
            {
                return new ConnectionHolderImpl( dataSetExecutor.getRiderDataSource()
                    .getDBUnitConnection()
                    .getConnection() );
            }
            else
    ...
    private static ConnectionHolder getConnectionFromTestClass( ExtensionContext extensionContext,
        String executorId )
    {
        DataSetExecutor dataSetExecutor = DataSetExecutorImpl.getExecutorById( executorId );
        if( isCachedConnection( dataSetExecutor ) )
    ...
    private static ConnectionHolder findConnectionFromTestClass( ExtensionContext extensionContext,
        Class< ? > testClass )
    {
        try
        {
            ...
            if( fieldFound.isPresent() )
            {
                ...
                Object testInstance = Modifier.isStatic( field.getModifiers() ) ? null
            ...
            if( methodFound.isPresent() )
            {
                ...
                ConnectionHolder connectionHolder =
                    (ConnectionHolder)method.invoke( extensionContext.getRequiredTestInstance() );
                if( connectionHolder == null )
                {
                    throw new RuntimeException( "ConnectionHolder not initialized correctly" );
                }
    ...
    {
        return executor != null && executor.getDBUnitConfig()
            .isCacheConnection();
    }
