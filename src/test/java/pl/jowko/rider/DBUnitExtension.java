...
import static pl.jowko.rider.ConnectionManager.getConfiguredDataSourceBeanName;
import static pl.jowko.rider.ConnectionManager.getTestConnection;
import static pl.jowko.rider.Constants.EMPTY_STRING;
import static pl.jowko.rider.Constants.JUNIT5_EXECUTOR;
import static pl.jowko.rider.Constants.NAMESPACE;
    ...
    BeforeEachCallback, AfterEachCallback, BeforeAllCallback, AfterAllCallback
{
    ...
    public void beforeTestExecution( ExtensionContext extensionContext ) throws Exception
    {
        ...
        final DBUnitConfig dbUnitConfig = resolveDbUnitConfig( Optional.empty(),
            extensionContext.getTestMethod(), extensionContext.getRequiredTestClass() );
        ...
        if( dbUnitConfig.isLeakHunter() )
        {
            ...
            {
                LOG.warn( format( "Could not create leak hunter for test %s",
                    extensionContext.getRequiredTestMethod()
                        .getName() ),
                    e );
            }
        }
        RiderTestContext riderTestContext =
            new JUnit5RiderTestContext( dbUnitTestContext.getExecutor(), extensionContext );
    ...
    public void afterTestExecution( ExtensionContext extensionContext ) throws Exception
    {
        ...
        final DBUnitConfig dbUnitConfig = dbUnitTestContext.getExecutor()
            .getDBUnitConfig();
        RiderTestContext riderTestContext =
            new JUnit5RiderTestContext( dbUnitTestContext.getExecutor(), extensionContext );
        RiderRunner riderRunner = new RiderRunner();
    ...
    private DBUnitTestContext getTestContext( ExtensionContext context )
    {
        ...
        return store.getOrComputeIfAbsent( testClass, ( tc ) -> createDBUnitTestContext( context ),
    ...
    private Set< Method > findCallbackMethods( Class testClass, Class callback )
    {
        ...
        Stream.of( testClass.getSuperclass()
            ...
            .forEach( m -> methods.add( (Method)m ) ); // do not use Collectors.toSet here: stream
                                                       // incompatible types
    ...
    public void beforeEach( ExtensionContext extensionContext ) throws Exception
    {
        if( extensionContext.getTestClass()
            .isPresent() )
        {
            Set< Method > callbackMethods = findCallbackMethods( extensionContext.getTestClass()
                .get(), BeforeEach.class );
        ...
        Method callbackMethod ) throws SQLException
    {
        ...
        Optional< DataSet > dataSetAnnotation =
            AnnotationUtils.findAnnotation( callbackMethod, DataSet.class );
        ...
        {
            Optional< DataSet > classLevelDataSetAnnotation =
                AnnotationUtils.findAnnotation( testClass, DataSet.class );
    ...
    /**
     * We only need to close the connection in afterCallback because the connection opened in before callback
     * is closed after test execution ({@link RiderRunner#teardown(RiderTestContext)})
     *
        ...
        throws SQLException
    {
        ...
        if( !dataSetExecutor.getDBUnitConfig()
            .isCacheConnection()
            && !dataSetExecutor.getRiderDataSource()
                .getDBUnitConnection()
                .getConnection()
                .isClosed() )
        {
    ...
    private void executeExpectedDataSetForCallback( ExtensionContext extensionContext,
        Class callbackAnnotation, Method callbackMethod ) throws DatabaseUnitException, SQLException
    {
        Class testClass = extensionContext.getTestClass()
            .get();
        // get ExpectedDataSet annotation, if any
        Optional< ExpectedDataSet > expectedDataSetAnnotation =
            AnnotationUtils.findAnnotation( callbackMethod, ExpectedDataSet.class );
        if( !expectedDataSetAnnotation.isPresent() )
        {
            LOG.warn( "Could not find expectedDataSet annotation annotation from callback method: "
                + callbackMethod );
            return;
        }
        ...
        DBUnitConfig dbUnitConfig = resolveDbUnitConfig( Optional.of( callbackAnnotation ),
            Optional.of( callbackMethod ), testClass );
        ...
        dataSetExecutor.compareCurrentDataSetWith(
            ...
            expectedDataSet.ignoreCols(), expectedDataSet.replacers(), expectedDataSet.orderBy(),
            expectedDataSet.compareOperation() );
        closeConnectionForAfterCallback( dataSetExecutor, callbackAnnotation );
    }

    private DataSetExecutor resetExecutorConnectionIfNeeded( ExtensionContext extensionContext,
        Class callbackAnnotation, DBUnitConfig dbUnitConfig, DataSetExecutor dataSetExecutor )
    {
        if( !dbUnitConfig.isCacheConnection() && isAfterTestCallback( callbackAnnotation ) )
        { // we close the connection after test execution when cache is disabled so we need a new one for the
          // callback
            final ConnectionHolder connectionHolder =
                getTestConnection( extensionContext, dataSetExecutor.getExecutorId() );
            dataSetExecutor = DataSetExecutorImpl.instance( dataSetExecutor.getExecutorId(), connectionHolder,
                dbUnitConfig );
        }
        ...
        Class testClass )
    {
        ...
        if( !dbUnitAnnotation.isPresent() && callbackAnnotation.isPresent() )
        {
            ...
            if( !callbackMethods.isEmpty() )
            {
                dbUnitAnnotation = AnnotationUtils.findAnnotation( callbackMethods.iterator()
                    .next(), DBUnit.class );
            }
        }
        ...
        return dbUnitAnnotation.isPresent() ? DBUnitConfig.from( dbUnitAnnotation.get() )
    ...
    private String getExecutorId( final ExtensionContext extensionContext, DataSet dataSet )
    {
        ...
        return annDataSet.map( DataSet::executorId )
    ...
    private Optional< DataSet > findDataSetAnnotation( ExtensionContext extensionContext )
    {
        ...
        {
            Optional< DataSet > annDataSet =
                AnnotationUtils.findAnnotation( testMethod.get(), DataSet.class );
            if( !annDataSet.isPresent() )
            {
                annDataSet =
                    AnnotationUtils.findAnnotation( extensionContext.getRequiredTestClass(), DataSet.class );
            }
            return annDataSet;
        }
        else
